...
function onJoinCommand(msg) {
  ...
  var name = msg.params[0];
  var chan = channel(name);
  var user = msg.from;
  var self = user.id === this.user.id;
  if (self) {
    chan.client = this;
...
function onKickCommand(msg) {
  ...
  var chans = msg.params[0].split(",");
  var users = msg.params[1].split(",");
  let i = 0, j = 0;
  let k = users.length;
  let l = chans.length;
  let user = null;
...
function onModeCommand(msg) {
  ...
  var target  = param === this.user.nick ? this.user :
                  this.channels.get(id(param)) || person(param);
  var modes   = parser.parseMode(msg.params[1].replace(/^:/, ""));
  if (!target) {
...
function onPartCommand(msg) {
  ...
  var chan = this.channels.get(id(name));
...
function onPingCommand(ping) {
  var reply = message(COMMAND.PONG, ping.params);
  this.send(reply);
  return STATUS.SUCCESS;
}

function onTopicCommand(msg) {
  var chan  = this.channels.get(id(msg.params[0]));
  var topic = msg.params[1].slice(1);
  if (chan) {
    if (chan.topic) {
...
function onNameReply(msg) {
  var chan  = this.channels.get(id(msg.params[2]));
  var nicks = msg.params[3].split(" ");
  var count = nicks.length;
  if (!chan) {
...
function onTopicReply(msg) {
  var chan  = this.channels.get(id(msg.params[1]));
  var topic = msg.params[2].slice(1);
  if (chan) {
    log.debug("Setting topic for %s to %s", chan, topic);
    chan.topic = topic;
    return STATUS.SUCCESS;
