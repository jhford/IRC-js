...
var person        = require("./person").person;
var server        = require("./server").server;
var trailing      = require("./message").trailing;
...
var REPLY         = constants.REPLY;
var STATUS        = constants.STATUS;
var SOCKET        = constants.SOCKET;

/** Maximum message length allowed by IRC */
var MESSAGE_MAX_LENGTH  = 512;
var MESSAGE_BUFFER_SIZE = MESSAGE_MAX_LENGTH + 4;

// Keys for config
var CFG_ADDRESS       = "address";
var CFG_CHANNELS      = "channels";
var CFG_DIE           = "die";
var CFG_LOG           = "log";
var CFG_MODE          = "mode";
var CFG_NICK          = "nick";
var CFG_PASSWORD      = "password";
var CFG_PORT          = "port";
var CFG_SSL           = "ssl";
var CFG_REALNAME      = "realname";
var CFG_SERVER        = "server";
var CFG_USER          = "user";
var CFG_USERNAME      = "username";
var CFG_FLOOD_PROTECT = "flood-protection";

// Level is (re)set later, when config is read
var log = logger.get("ircjs", LEVEL.ALL);
...
function backoff(f, ratio) {
  ...
  return function() {
    ...
    function later() {
      var now  = Date.now();
      var wait = interval - (now - lastCall);
      if (wait > 0) {
...
function Client(conf) {
  var config = makeConfig(mergeObjects(defaultConfig, conf));
  var server = config.get(CFG_SERVER);
 ...
 */
function onConnect() {
  var user = this.config.get(CFG_USER);
  var mode = user.has(CFG_MODE) ?
    parser.parseMode(user.get(CFG_MODE)) : null;
  var password = user.get(CFG_PASSWORD);
...
Client.prototype.disconnect = function() {
  var connected = this.connectedAt;
  var connectedSince = connected.getTime();
  connected.setTime(0);
  this.socket.end();
  this.socket = null;
...
function onData(data) {
  var prev = this.streamBuffer;
  if (prev) {
    var newBuf = new Buffer(prev.length + data.length);
    prev.copy(newBuf);
  ...
  for (let i = 0, j = 0, l = data.length; i < l; ++i) {
    ...
    var msg = parser.parse(data.slice(j, i + 1));
    msg.client = this;
...
Client.prototype.connect = function(callback) {
  ...
  var sock = this.socket = this.server.connect();
  sock.setTimeout(0);
...
Client.prototype.send = function(message) {
  ...
  var remains = buffer.readUInt16LE(2);
...
Client.prototype.quit = function(reason) {
  var params = reason ? [trailing(reason)] : [];
...
Client.prototype.part = function(chan, msg) {
  var ch = this.channels.get(chan.id || id(chan));
  if (!ch) {
    return this;
  }
  var params = [ch.name];
...
function matchRegExp(re, handler) {
  ...
  return matchType.call(this, COMMAND.PRIVMSG, function(msg) {
    var match = msg.params[1].match(re);
    if (pred && !pred(msg)) {
      return null;
    }
    if (!match) {
...
Client.prototype.ignore = function(type, handler) {
  var arr = this.handlers.get(type);
  if (!arr) {
    return 0;
  }
  var ix = arr.indexOf(handler);
  if (-1 === ix) {
    return arr.length;
  }
  arr.splice(ix, 1);
...
Client.prototype.notify = function(type /*, arg1, arg2 ... argN */) {
  ...
  for (let i = 0, l = handlers.length; i < l; ++i) {
    var status = handlers[i].apply(this, args);
