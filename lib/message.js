...
function forMe(msg, nick) {
  var msgText   = msg.params[1];
  var hasNick   = new RegExp(format("^:\\s*%s\\b", nick), "i").test(msgText);
  var hasPrefix = prefixRE.test(msgText);
  var isQuery   = msg.params[0] === nick;
...
Message.prototype.reply = function(text) {
  var sender  = this.params[0];
  var recip   = sender === this.client.user.nick ? this.from.nick : sender;
  var params  = [recip];
  ...
  var msg = message(COMMAND.PRIVMSG, params);
